...
///The 1st fseek function moves 2 position from the current position and the next seek function again moves 2 position from the then current position.
    fseek(fp,-1,2);///This back to one position left from the last of Pranab. Here the 3rd position's 2 indicates the last of Pranab.
    fscanf(fp,"%s",str);
///Here after scanf up-downing line is presented at the last.
    puts(str);
    fseek(fp,-2,2);///Here it backs 2 position left from the last.
    fscanf(fp,"%s",str);
    puts(str);
